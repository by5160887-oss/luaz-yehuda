...
export default async function(req,res){
 ...
 if(a==='toggle_daily'){const date=req.body?.date,itemId=Number(req.body?.itemId);if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!Number.isInteger(itemId)||itemId<0||itemId>18)return res.status(400).json({error:'invalid_daily_action'});data.planner||={};if(data.planner.daily?.date!==date)data.planner.daily={date,completed:{}};data.planner.daily.completed[itemId]=!!req.body.done}
 else if(a==='save_planner'){const p=req.body?.planner;if(!p||typeof p!=='object'||Array.isArray(p)||JSON.stringify(p).length>25000)return res.status(400).json({error:'invalid_planner'});data.planner=p}
 else if(a==='toggle_task'){if(!found)return res.status(404).json({error:'not_found'});found.t.done=!!req.body.done}
