...
async function connect(){const sql=neon(process.env.DATABASE_URL);await sql`CREATE TABLE IF NOT EXISTS app_state(id text PRIMARY KEY,data jsonb NOT NULL,updated_at timestamptz DEFAULT now())`;await sql`INSERT INTO app_state(id,data) VALUES('main',${JSON.stringify(seed)}::jsonb) ON CONFLICT(id) DO NOTHING`;return sql}export{connect};export default async function(req,res){try{const sql=await connect(),[r]=await sql`SELECT data,updated_at FROM app_state WHERE id='main'`;res.json({...r.data,updatedAt:r.updated_at})}catch(e){res.status(500).json({error:'state_unavailable'})}}